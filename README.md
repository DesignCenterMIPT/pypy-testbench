...
## Запуск микротестов
В этой части мы запускаем микротесты, направленные на тестироване времени работы определенных частей бэнчмарков.
### Предобработка микротестов
#### Список библеотек
Модули, которые неоходимо скачасть для работы с тестами:
 * sympy
 * pyperfomance
#### Необходимые поправки
 * Вынести все файлы микротестов в отедьную директорию, лучше всего при помощи hardlinks.
 * В тестах ввести полный путь до необходимых файлов данных.
 * В случае если необходимо будет запустить дамп с профайлетами, вополнить два предыдущих пункта и для этих файлов.
 * Желательно оставить папку для логов.
### Запуск микротестов
Для их запуска микротестов следует использовать следующую команду
```
./test-jit-dump.sh path/to/test/root
```
Важно чтобы ```<path/to/test/root>``` имел следующее дерево
```
root
|_ test1
|   |_ test-one.py
...
...
```
Тогда скрипт отработает корректно. 

Если вы используете основной репозиторий для выполнения этого скрипта можно запустить скрипт без аргумента. В скрипт встроен дефолтный путь. 

Что делает скпипт? Он проходится по всем тестам вида `test-<name>.py` и записывает логи в файл `jit-backend-<name>.dump`. После полной записи, он перекладывает логи в директорию `microtest-logs`

Далее логи можно прогнать через `codeview.py` который выведем дизаасембл-код. Для этого следует использовать `jit-view.sh`, в качестве аргумента следует подавать сам файл лога с раширением `.dump`